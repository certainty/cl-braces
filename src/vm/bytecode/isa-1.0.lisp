...
(define-isa *isa-1.0*
  ...
  (
   ...
   (#x41 jmp   ((addr dst))                    "Unconditionally jump to $dst")
   ...
   (#x82 cmp   ((reg lhs) (reg rhs))           "Compares $lhs to $rhs and sets the flags accordingly.")

   ;; ternary instructions #xc0 - #xff
   (#xC0 br    ((addr iftrue) (addr iffasle) (reg value)) "Branches to $iftrue when $value is true and to $iffalse otherwise")
