...
(define-test adding-symbols ()
  (let ((st (symbols:make-symbol-table)))
    (assert-false (symbols:find-by-name st "foo"))
    (symbols:add-symbol st "foo" :variable)
    (assert (symbols:find-by-name st "foo"))))

(define-test finding-symbols-by-id ()
  (let ((st (symbols:make-symbol-table)))
    (symbols:add-symbol st "bar" :variable)
    (symbols:add-symbol st "foobar" :function)

    (let* ((id (symbols:add-symbol st "foo" :variable))
...
(define-test finding-symbol-by-name  ()
  (let ((st (symbols:make-symbol-table)))
    (symbols:add-symbol st "bar" :variable)
    (symbols:add-symbol st "foobar" :function)
    (symbols:add-symbol st "foobar" :variable)
    (symbols:add-symbol st "foo" :variable)
    (let* ((found (symbols:find-by-name st "foo")))
      (assert-equal (symbols:denotation (first found)) :variable)
      (assert-equal (symbols:name (first found)) "foo"))

    (let* ((found (symbols:find-by-name st "foobar")))
...
(define-test find-by-name-and-denotation ()
  (let ((st (symbols:make-symbol-table)))
    (symbols:add-symbol st "bar" :variable)
    (symbols:add-symbol st "bar" :variable :scope 3)
    (symbols:add-symbol st "bar" :variable :scope 4)
    (symbols:add-symbol st "bar" :function)
    (symbols:add-symbol st "foo" :function)

    (let ((found (symbols:find-by-name st "bar" :denotation (symbols:denotes-any :function :variable))))
      (assert-true (consp found))
      (assert-equal (length found) 4))

    (let ((found (symbols:find-by-name st "bar" :denotation #'symbols:denotes-function-p)))
      (assert-true (consp found))
...
(define-test find-by-name-denotation-and-scope ()
  (let ((st (symbols:make-symbol-table)))
    ...
    (let ((found (symbols:find-by-name st "bar" :denotation (symbols:denotes-any :variable))))
      (assert-true (consp found))
    ...
    (let ((found (symbols:find-by-name st "bar" :denotation #'symbols:denotes-variable-p :scope<= 2)))
      (assert-true (consp found))
      (assert-equal (length found) 1)
